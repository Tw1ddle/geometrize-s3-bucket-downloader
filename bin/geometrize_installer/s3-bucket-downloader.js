...
var BucketConfig = function() {
	...
	this.EXCLUDE_FILES = ["index.html","s3-bucket-downloader.js","style.min.css","sortable.min.js"];
...
var BucketListing = function(config,navigationContainer,listingTableContainer,loadingSpinner,refreshButton) {
	...
	this.refreshButton.onclick = function() {
		var tmp = _gthis.getQueryUrlForRootDirectory();
		_gthis.requestData(tmp);
	};
};
BucketListing.__name__ = ["BucketListing"];
BucketListing.prototype = {
	requestData: function(bucketRequestUrl) {
		var _gthis = this;
		...
		var http = new haxe_Http(bucketRequestUrl);
	...
	,getQueryUrlForRootDirectory: function() {
		var baseUrl = this.config.BUCKET_URL + "?list-type=2" + "&delimiter=/";
		return baseUrl;
	}
	,getQueryUrlForDirectory: function(directoryPath) {
		var url = this.getQueryUrlForRootDirectory();
		if(directoryPath.length == 0) {
			return url;
		}
		url += "&prefix=" + encodeURIComponent(directoryPath);
		return url;
	}
	,makeAnchorLinkForFile: function(text,filePath) {
		var anchor = window.document.createElement("a");
		anchor.innerText = text;
		anchor.href = this.config.BUCKET_URL + filePath;
		return anchor;
	}
	,makeButtonForDirectory: function(text,directoryPath) {
		var _gthis = this;
		var button = window.document.createElement("button");
		button.innerText = text;
		button.onclick = function() {
			var tmp = _gthis.getQueryUrlForDirectory(directoryPath);
			_gthis.requestData(tmp);
		};
		return button;
	}
	...
	,buildNavigation: function(info) {
		...
		var addButton = function(text,directoryPath) {
			var button = _gthis.makeButtonForDirectory(text,directoryPath);
			navigation.appendChild(button);
		};
		var parts = prefix.length == 0 ? [""] : prefix.split("/");
		addButton("root","");
		var partsChain = "";
		var i = 0;
	...
	,generateListingTable: function(info) {
		...
		var makeRowForFile = function(file) {
			...
			iconTypeCell1.className = "bucket_listing_file_icon_cell";
			var nameCell1 = window.document.createElement("td");
			nameCell1.className = "bucket_listing_file_name_cell";
			var fileLink = _gthis.makeAnchorLinkForFile(file.fileName,file.fileName);
			...
			modifiedDateCell1.className = "bucket_listing_file_modified_date_cell";
			var sizeCell1 = window.document.createElement("td");
			sizeCell1.textContent = Util.formatBytes(parseFloat(file.size),2);
			sizeCell1.className = "bucket_listing_file_size_cell";
		...
		var makeRowForDir = function(dir) {
			...
			iconTypeCell2.className = "bucket_listing_dir_icon_cell";
			var nameCell2 = window.document.createElement("td");
			nameCell2.className = "bucket_listing_dir_name_cell";
			var dirLink = _gthis.makeButtonForDirectory(dir.prefix,dir.prefix);
			nameCell2.appendChild(dirLink);
			var modifiedDateCell2 = window.document.createElement("td");
			modifiedDateCell2.textContent = "-";
			modifiedDateCell2.className = "bucket_listing_dir_modified_date_cell";
			var sizeCell2 = window.document.createElement("td");
			sizeCell2.textContent = "-";
			sizeCell2.className = "bucket_listing_dir_size_cell";
...
Main.prototype = {
	onWindowLoaded: function() {
		this.bucket.requestData(this.bucket.getQueryUrlForRootDirectory());
	}
	,__class__: Main
};
Math.__name__ = ["Math"];
